...
if ($method === 'GET') {
    ...
    $query = "SELECT * FROM salud ORDER BY id DESC";
    ...
    // Obtener los valores
    $vacunado = $input_data['vacunado'];
    $desparasitado = $input_data['desparasitado'];
    $sano = $input_data['sano'];
    $esterilizado = $input_data['esterilizado'];
    $identificado = $input_data['identificado'];
    $microchip = $input_data['microchip'];
    $descripcion = $input_data['descripcion'];
    $id_mascota = $input_data['id_mascota'];

    // Insertar el nuevo registro en la base de datos
    $query = "INSERT INTO salud (vacunado, desparasitado, sano, esterilizado, identificado, microchip, descripcion, id_mascota) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $connect->prepare($query);
    $stmt->execute([$vacunado, $desparasitado, $sano, $esterilizado, $identificado, $microchip, $descripcion, $id_mascota]);
    ...
    if (isset($data['id'])) {
        ...
        $query = "DELETE FROM salud WHERE id=?";
        $stmt = $connect->prepare($query);
        $stmt->execute([$id]);
...
/*
{
  ...
  "identificado": false,
  "microchip": true,
  "descripcion": "Descripción de la mascota aquí",
  "id_mascota": 12345
}
*/